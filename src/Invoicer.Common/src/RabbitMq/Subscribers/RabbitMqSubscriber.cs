    ...
    internal sealed class RabbitMqSubscriber : IBusSubscriber
    {
            ...
            where T : class
        {
            ...
            if (declare)
            {
                if (_loggerEnabled)
                {
                    _logger.LogInformation("Declaring a queue: '{queueName}' with routing key: '{routingKey}' "
                                           +"for an exchange: '{exchangeName}'.", 
                        conventions.Queue, conventions.RoutingKey, conventions.Exchange);
                }
            ...
            if (_options.DeadLetter?.Enabled is true)
            {
                if (_options.DeadLetter.Declare)
                {
                    ...
                    _logger.LogInformation("Declaring a dead letter queue: '{deadLetterQueue}' " +
                        ...
                        deadLetterQueue, deadLetterExchange, ttl);
                ...
                try
                {
                    ...
                    if (_loggerEnabled)
                    {
                        _logger.LogInformation("Received a message with id: '{messageId}', " +
                                               ...
                                               $"{(_loggerEnabled ?"[queue: '{Queue}', routing key: '{RoutingKey}', " + "exchange: '{Exchange}']" : String.Empty)}.", 
                            messageId, correlationId, timestamp, conventions.Queue, conventions.RoutingKey, conventions.Exchange);
                    }
            ...
            await retryPolicy.ExecuteAsync(async () =>
            {
                try
                {
                    ...
                    if (_loggerEnabled)
                    {
                        ...
                        _logger.LogInformation(preLogMessage,messageName, messageId,correlationId, retryMessage, currentRetry);
                    }
                    ...
                    if (_loggerEnabled)
                    {
                        var postLogMessage = "Handled a message: '{messageName}' [id: '{messageId}'] " +
                                             "with correlation id: '{correlationId}'. {retryMessage}";
                        _logger.LogInformation(postLogMessage, messageName, messageId, correlationId, retryMessage);
                    }
                }
                catch (Exception ex)
                {
                    ...
                    if (rejectedEvent is null)
                    {
                        var errorMessage = "Unable to handle a message: '{messageName}' [id: '{messageId}'] " +
                                           ...
                                           "retry {currentRetry}/{retries}...";

                        if (currentRetry > 1)
                        {
                            _logger.LogError(errorMessage,messageName,messageId,correlationId, currentRetry - 1, _retries);
                        }
                    ...
                    if (_loggerEnabled)
                    {
                        _logger.LogWarning("Published a rejected event: '{rejectedEventName}' " +
                            ...
                            rejectedEventName, messageName,messageId, correlationId);
                    }

                    _logger.LogError("Handling a message: '{messageName}' [id: '{messageId}'] " +
                                     "with correlation id: '{correlationId}' failed and rejected event: " +
                                     "'{rejectedEventName}' was published.", messageName, messageId, correlationId, rejectedEventName,  ex);
